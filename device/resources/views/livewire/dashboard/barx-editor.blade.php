<div class="space-y-6">
    <div>
        <h2 class="text-lg font-semibold text-white">Barx Environment Config</h2>
        <p class="text-gray-400 text-sm mt-0.5">Manage environment variables and PATH modifications for Barx AI tools.</p>
    </div>
    ...
    @if (count($backups) > 0)
        <div class="bg-white/[0.02] rounded-xl border border-white/[0.06] p-4">
            <div class="flex items-center gap-4">
                <div class="flex-1">
                    <label class="block text-sm font-medium text-gray-400 mb-2">Restore from Backup</label>
                    <select
                        wire:model="selectedBackup"
                    ...
                    >
                        ...
                        @foreach ($backups as $backup)
                            <option value="{{ $backup['path'] }}">{{ $backup['label'] }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="pt-6">
                    <button
                        ...
                        wire:target="restore"
                        @disabled($isReadOnly || empty($selectedBackup))
                        class="px-4 py-2 bg-white/[0.06] hover:bg-white/10 disabled:opacity-50 disabled:cursor-not-allowed text-gray-400 hover:text-white text-sm rounded-lg transition-colors"
                    >
    ...
    <div class="bg-white/[0.02] rounded-2xl border border-white/[0.06] p-6 space-y-4">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-xs text-gray-500 mt-1">Variables will be added to ~/.barx with export statements.</p>
            </div>
            @if ($isDirty)
                <span class="text-xs text-amber-400 bg-amber-500/10 px-2 py-1 rounded">Unsaved changes</span>
            @endif
    ...
    <div class="flex items-center justify-between pt-2">
        <div class="flex items-center gap-3">
            <button
                ...
                wire:confirm="Reset all environment variables? This will remove the VibeCodePC section from ~/.barx."
                type="button"
                @disabled($isReadOnly)
    ...
    <div class="text-xs text-gray-600 px-2">
        File: <code class="text-gray-500">{{ $barxPath }}</code>
        @if (file_exists($barxPath))
            @php
                $stat = stat($barxPath);
                if ($stat) {
